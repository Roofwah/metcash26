...
const BRAND_LOGOS: [string, string][] = [
  ['ARMOR ALL',  '/logos/aall.png'],
  ['JELLY BELLY','/logos/jelly.png'],
  ['ENERGIZER',  '/logos/energizer.png'],
  ['EVEREADY',   '/logos/eready.png'],
];

function loadAllLogos(): Promise<LogoMap> {
  if (logoCacheMap) return Promise.resolve(logoCacheMap);
  return Promise.all(
    BRAND_LOGOS.map(([brand, src]) =>
      new Promise<[string, HTMLImageElement]>((resolve) => {
        const img = new Image();
        img.onload  = () => resolve([brand, img]);
        img.onerror = () => resolve([brand, img]); // silent fail
        img.src = src;
      })
    )
  ).then(entries => {
...
function ease(t: number): number {
  if (t < 0.08) { const u = t / 0.08; return u * u * 0.05; }
  const u = (t - 0.08) / 0.92;
...
function drawFrame(
  ...
  for (let i = 0; i < SEGMENT_COUNT; i++) {
    ...
    if (logo?.naturalWidth) {
      ...
      const logoR    = outerR * 0.80;
      const radialMax = outerR * 0.36;
      const tangMax   = 2 * logoR * Math.sin(SEGMENT_ANGLE / 2) * 0.80;
  ...
  // ── Hub logo — energizer.png (already at 45° angle in the image) ──
  ...
  if (hubLogo?.naturalWidth) {
    ...
    ctx.clip();
    // multiply: white bg of PNG becomes transparent over the hub
...
const SpinWheel = forwardRef<SpinWheelHandle, Props>(function SpinWheel({ onWin }, ref) {
  ...
  useEffect(() => {
    ...
    resize();
    loadAllLogos().then(m => { logosRef.current = m; });
  ...
  const dismiss = useCallback(() => { setShowModal(false); setWinner(null); }, []);
    ...
    <div className="sw-wrapper">
      <div className="sw-canvas-area">
        <canvas
          ref={canvasRef}
          className="sw-canvas"
          onClick={!spinning && !showModal ? spin : undefined}
        />
          ...
          <div className="sw-modal" onClick={e => e.stopPropagation()}>
            ...
            {logosRef.current?.[winner.brand] && (
              <div className="sw-modal-logo-wrap" style={{ background: '#fff' }}>
                <img
                  src={logosRef.current[winner.brand].src}
                  alt={winner.brand}
                  className="sw-modal-logo"
                />
              </div>
            )}
            <div className="sw-modal-prize">{winner.name}</div>
            <div className="sw-modal-sku">SKU: {winner.sku}</div>
            <button className="sw-modal-btn" onClick={dismiss}>CLAIM PRIZE</button>
          </div>
        </div>
      )}
    </div>
